...
- Controller mode can paste an image into a reply or a new session's first prompt to send it along, search sessions by title, project, or id, approve the commits an agent proposes, open a real terminal on the host in the session's own directory, with colour, full-screen programs and tab completion, start a session that continues under the one you have open or on a fresh branch, ask for shorter agent replies, steer a session by tapping the agent's ranked next steps instead of typing, compact a long draft prompt down to shorthand or expand terse shorthand into a full prompt before sending it, summarize the last reply into a new note you can answer from, and disguise itself as an open editor — file-path session names and a page of source until you point at the pane — or as a text-cleaning utility (a plain document you type into, controls tucked into the sidebar as cleaners) or a code-editor buffer (your prompts as added lines, replies as comments, the composer as the last line).
- Controller mode can read the agent's replies aloud — a few spoken sentences on what it did and what it needs, ending with numbered answers you can tap, only when it is waiting on you or for every reply — now in a choice of natural voices (a free neural voice, or your own OpenAI voice) and in English or Egyptian Arabic, and asks the agent to end with numbered options so those answers are real ones.

### Changed
<!-- Changes to existing functionality go here -->
- Controller mode is quieter to look at and easier to live in — replies arrive as plain text with the Markdown syntax stripped, selecting any text in the transcript copies it straight to the clipboard with a brief confirmation, images paste or drop into the text-cleaner and code-editor composers too, a header and composer whose controls stay out of sight until you reach for them, collapsible project groups ordered by the workspace you touched last, a marker on sessions waiting for your answer, a session list you can drag to resize or fold to a rail, a popover you can resize (with a reset back to the default size) or pin open so it stops vanishing when you click away, a choice of how the transcript hides for a public glance (a uniform dim, a reading light that follows the cursor, per-message blur, or a page of source), keystrokes to distill a reply, drop a quick reply into the composer, or hide it all at once, a choice of font and text size, links that open in your browser, file references you can tap to read the real file from the host, selectable text anywhere in the transcript, and keys to jump through a long one.
