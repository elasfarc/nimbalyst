...
export class TrayManager {
  ...
  private createTray(): void {
    ...
    } else if (isTrayPanelSupported()) {
      // Where the panel is supported, the sessions live in it and the NSMenu is
      // reduced to app actions on right-click. Elsewhere `refreshMenuBar`
      // installs the full session menu via `setContextMenu` and these handlers
      // never run.
  ...
  private refreshMenuBar(): void {
    ...
    this.appMenu = menu;
    if (isControllerMode()) {
      // CTRL-05: in controller mode the tray is a click-to-toggle popover
      // button, so we leave the context menu unset and let the click handler
      // fire (right-click pops up the controller menu explicitly).
